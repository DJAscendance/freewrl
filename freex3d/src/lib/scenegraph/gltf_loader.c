...
struct geomBuffer* find_or_create_buffer_add_user(void* ectx, const cgltf_buffer_view* bv) {
	struct geomBuffer* buffer = find_buffer_in_broto_context_from_cgltf_buffer(ectx, bv->buffer);
	if (!buffer) {
		//first use of buffer, allocate
		buffer = add_geomBuffer0(ectx, bv->buffer->size, 1);
		buffer->cgltf_buffer = (char*)bv->buffer;
		if (bv->buffer->data) {
			memcpy(buffer->address, bv->buffer->data, bv->buffer->size); //deep copy, do we need deep?
			buffer->loaded = 1;
			buffer->cgltf_buffer = (char*)bv->buffer; //simpler address copy
		}
	}
	else {
		add_geomBufferUser(buffer);
	}
	return buffer;
}

struct X3D_Node *x3dtexture_from_cgltf_texture(struct X3D_Node *ectx, cgltf_texture* texture ){
	char* iname = texture->image->name;
	struct X3D_Node* image = USE_node(iname, X3DTextureNode);
	if (!image) {
		if (texture->image->buffer_view) { //->buffer->data){
			image = DEF_node(ectx, iname, NODE_BufferTexture);
			struct X3D_TextureRep* tr = set_TextureRep(NULL);
			tr->buffer = find_or_create_buffer_add_user(ectx, texture->image->buffer_view);
			tr->byteOffset = texture->image->buffer_view->offset;
			tr->byteSize = texture->image->buffer_view->size;
			image->_intern = (void*)tr;
		}
		else {
			char* iuri = texture->image->uri;
				...
				switch(prim->type){
					...
					case cgltf_primitive_type_triangles:
					{
						...
						for (int ii = 0; ii < acount; ii++) {
							...
							//}
							mr->buffer = find_or_create_buffer_add_user(ectx, blob->buffer_view);
...
int gltf_load_bin(resource_item_t *res){
	...
	cgltf_buffer* buffer = (void*)res->resm_specific;
	if (buffer) {
