...
// BAD EXPERIMENTAL RE-ARRANGING DOESN'T WORK
// goal: proper use of glu tess combiner to interpolate per-vertex fog, texcoord, normal, color/colorRGBA
// a) any normal / color / texcoord / fog fiddling move ahead of tesselation, so combiner has it all
// b) once in tess loop generating triangles, don't every look back at any original index arrays - because those won't have combiner-generated point data
//void make_genericfaceset(struct X3D_IndexedFaceSet *node) {
void make_genericfaceset_EXPERIMENTAL(struct X3D_IndexedFaceSet *node) {
	int cin;
	int cpv = TRUE;
	int npv;
	int tcin;
