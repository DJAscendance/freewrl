...
int overMenubar(ppstatusbar p, int mouseY){
	...
	if(p->showButtons){
		...
		if(p->pmenu.top)
			...
			y = mouseY - p->pmenu.yoffset;
		if( y >= 0 && y <= p->buttonSize * p->buttonRows) isOver = 1;
	}
	return isOver;
}
int overStatusbar(ppstatusbar p, int mouseY){
	...
	if(mouseY < p->statusBarSize * p->statusBarRows) isOver = 1;
	//if(p->show_status){
...
int statusbar_getClipPlane(){
	...
	statusbar_height = (p->statusbar_pinned && p->wantStatusbar)? p->statusBarSize * p->statusBarRows : 0;
	menubar_height   = (p->menubar_pinned   && p->wantButtons)  ? p->buttonSize    * p->buttonRows : 0;
	vrml_clipplane = statusbar_height + menubar_height;
...
//	if(!p->wantStatusbar) return;
	...
	p->pmenu.yoffset = p->show_status ? p->statusBarSize * p->statusBarRows : 0;

	//p->clipPlane is for statusbarHud to glClear an area the background color of the status and/or menubar
	p->clipPlane = (p->show_menu ?  p->buttonSize * p->buttonRows : 0) + p->show_status ? p->statusBarSize * p->statusBarRows : 0; //(p->show_status ? p->statusBarSize : 0);
