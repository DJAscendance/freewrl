...
int main (int argc, char **argv)
{
	...
	if(testing_argvs){
		...
		case 0:
		{
			//backend display thread; url on command line
			CdllFreeWRL *one;
			one = new CdllFreeWRL();
		...
		case 1:
		{
			//backend display thread, complex argv
			CdllFreeWRL *one;
			one = new CdllFreeWRL();
			one->onInitArgv(argc, argv, false);
			getchar();
		}
		break;
		case 2:
		{
			//frontend display thread, complex argv
			// CAN'T DO THIS unless you create your own opengl window creation in the frontend
			// it should be called 'frontend_owns_opengl_thread'
			CdllFreeWRL *one;
			one = new CdllFreeWRL();
			one->onInitArgv(argc, argv, true);
			for(;;){
				one->onDraw();
			}
		}
